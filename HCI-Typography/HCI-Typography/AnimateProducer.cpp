...
AnimateProducer::AnimateProducer(sf::Vector2f tPos, sf::Vector2f sPos) :
	m_paused{ false },
	m_currentKeyFrame{ 0 }
{
...
void AnimateProducer::update(sf::Time t)
{
	if (!m_paused)
	{
...
void AnimateProducer::render(sf::RenderWindow & window)
{
	window.draw(m_title);
	window.draw(m_sub);
}

void AnimateProducer::setKeyframe(int key, sf::Time dur, sf::Vector2f tPos, sf::Vector2f sPos)
{
